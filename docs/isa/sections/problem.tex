...
evaluator runs six scenarios and the switched screen runs one \SI{34}{ms} transition sequence; Deb ordering ranks the record. After 240 global and 60 local calls the
...
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{\input{figures/fig_workflow}}
...
\subsection{Thirty-three inequalities in engineering units}
For each scenario five inequalities $g_{s,j}\le0$ are evaluated separately:
\begin{equation}
\begin{aligned}
...
g_{s,5}&=\frac{\rho_{\ell,s}-1}{0.005}-1,\;\; \rho_{\ell,s}=\frac{E_{loss}/E_u}{(E_{loss}/E_u)_{\xi_0}} &&\text{loss intensity},
\end{aligned}
\label{eq:g}
\end{equation}
...
load pulse) is set to $-1$. The loss-intensity gate keeps the design
loss-aware: no scenario may convert energy less efficiently, per useful joule,
than the Bryson record $\xi_0$ by more than \SI{0.5}{\percent}.
These 30 inequalities are completed by three measured ones,
\begin{equation}
g_{31}=\frac{\max_{k}\,\mathrm{pp}(d)}{0.20}-1,\quad
g_{32}=\frac{\max_{k}|\bar e_{sw,k}|}{\SI{25}{mV}}-1,\quad
g_{33}=\frac{\max_t i_{L,sw}}{\SI{23}{A}}-1,
\label{eq:gsw}
\end{equation}
...
feasible when $\sum_j\max(g_j,0)\le10^{-4}$; the screen costs about \SI{12}{ms}
...
sequence. $\rho_{rip}$ is kept as a reported diagnostic only. Both pilots used
the zero-order-hold filter; the triangle-hold update \eqref{eq:foh}, adopted
after the equivalence audit of Section~\ref{sec:res:equiv}, removes most of the
limit-cycling designs by itself.

\paragraph{Useful-energy band} The earlier protocol also required
$|E_u/E_{u,\xi_0}-1|\le\SI{0.5}{\percent}$ in every scenario. We report this
quantity but no longer constrain it. Anchored to $\xi_0$, it penalizes the
removal of the \SI{9.8}{V} undershoot of $\xi_0$ on the 46$\to$24~V step,
because a controller that holds \SI{24}{V} delivers more energy than one that
undershoots; anchored to the ideally regulated task, it penalizes every
controller by 1--\SI{5}{\percent} on step-down scenarios, where the asynchronous
converter can only discharge through its load. With absolute overshoot,
load-deviation and steady-error gates the delivered task is already pinned,
and the loss intensity is normalized per useful joule, so under-delivery
cannot improve it.
...
evaluation (six averaged scenarios, the switched screen, 33 inequalities).
...
7 refineBO & shared-kernel Matérn-5/2 GPs on $J_s$ and $g_1,\dots,g_{33}$; pool of 1000 around the 10 Deb-best records, radius $0.04\to0.003$ & $\alpha=P_f(\mathrm{EI}+0.02\sigma_J)+10^{-4}P_f$, $P_f=\prod_j\Phi(-\mu_j/\sigma_j)$; restoration mode if no feasible incumbent & 60 calls\\
\bottomrule
\end{tabular}}
\end{table*}
...
identical protocol (evaluator, 33 inequalities, 7 algorithms, 30 seeds,
