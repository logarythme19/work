...
| E12 | **Défaut corrigé après campagne : feedforward calculé avec le plant simulé** | Le §1 impose un feedforward nominal figé. Le noyau v6.1 utilisait les paramètres du plant en cours (coins inclus) ; seul l'étage 2 est concerné. Correction : paramètres du feedforward passés à part, toujours nominaux. Impact mesuré (`scripts/impact_feedforward.py`) sur les 540 candidats retenus : toujours **0 faisable**, meilleures marges modifiées de moins de 0,006, écart médian +0,002 à +0,004. La recherche de l'étage 2 n'a pas été relancée. | Rapporté ; les empreintes de `_kernel.py`, `evaluate.py`, `trace.py` ne correspondent plus au protocole v6.1. |
